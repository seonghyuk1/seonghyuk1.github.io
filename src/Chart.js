...
function Chart() {
  ...
  useEffect(() => {
    axios
      .get(
        "https://raw.githubusercontent.com/kimdayeon37/barchart/main/hos_%E1%84%82%E1%85%A9%E1%84%8B%E1%85%AF%E1%86%AB1.json"
      )
      .then((res) => {
        setHos(res.data);
      });
  ...
  return (
    <div className={"row"} style={{ marginLeft: "2%" }}>
      ...
      <div style={{ height: "650px", overflow: "scroll" }}>
        <HospitalTable hospital={hos} ss={ss} />
      </div>
    </div>
  );
}
...
function HospitalTable(props) {
  ...
  props.hospital.forEach((m) => {
    if (
      m.진료과목내용명.indexOf(props.ss) === -1 &&
      m.사업장명.indexOf(props.ss) === -1 &&
      m.도로명전체주소.indexOf(props.ss) === -1
    ) {
      return;
    }
    //배열에 추가
...
//Memo 기능 사용
const H2 = React.memo(() => {
  return <h1 className={"text-center"}>의료기관 검색 🔍</h1>;
});
